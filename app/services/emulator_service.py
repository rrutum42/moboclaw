...
class EmulatorService:
    def __init__(
        ...
        self.warm_pool = WarmPool(self.store, self.settings, self._replenish_lock)
    ...
    async def provision(self, snapshot_id: str | None) -> ProvisionEmulatorResponse:
        ...
        warm_id = await self.store.pop_warm_idle()
        ...
        boot = await simulate_boot_seconds(from_warm_pool=from_warm, settings=self.settings)
        log.info(
    ...
    async def destroy_emulator(self, emulator_id: str, reason: str = "user_delete") -> None:
        ...
        await teardown_emulator(self.store, emulator_id, reason)
        self._schedule_replenish()
